<?php
/**
 * Plugin Name: WooCommerce Bulk Customer Email
...
function pmbulk_admin_page() {
    ...
    if ( isset($_POST['pmbulk_action']) && check_admin_referer('pmbulk_nonce_action', 'pmbulk_nonce') ) {
        $action = sanitize_text_field($_POST['pmbulk_action']);
        $action_draft_id = '';
        ...
        if ( $action === 'send_test' ) {
            $test_email = sanitize_email($_POST['pmbulk_test_email']);
        ...
        <?php if ( $queue_active ) : ?>
            <div class="pm-box pm-progress" style="padding: 20px; border-left: 4px solid #111;">
                ...
                <p><strong>Progress:</strong> <?php echo esc_html($queue_sent); ?> of <?php echo esc_html($queue_total); ?> emails sent 
                    (<?php echo $queue_total > 0 ? round(($queue_sent / $queue_total) * 100) : 0; ?>%)</p>
                ...
                <div style="background: #e5e7eb; height: 32px; border-radius: 8px; overflow: hidden; margin: 10px 0;">
                    <div style="background: #111; height: 100%; width: <?php echo $queue_total > 0 ? ($queue_sent / $queue_total) * 100 : 0; ?>%; transition: width 0.3s;"></div>
                </div>
...
function pmbulk_get_customer_emails() {
    ...
    // Query to get unique billing emails from all completed and processing orders
    $emails = $wpdb->get_col("
